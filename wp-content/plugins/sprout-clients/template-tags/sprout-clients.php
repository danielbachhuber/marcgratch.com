	...
	function sc_get_status_select( $client_id = 0 ) {
		...
		<div id="current_statuses_<?php echo (int) $client_id ?>" class="sc_current_statuses">
			<?php foreach ( $status_terms as $term ) : ?>
				...
				<?php printf( '<a href="%4$s" class="button button-small sc_status %2$s status_id_%3$s" title="%1$s">%1$s</a>', esc_attr( $term->name ), esc_attr( $current_status ), (int) $term->term_id, esc_url_raw( $url ) ); ?>
			<?php endforeach ?>
		</div>
		<?php
		$view = ob_get_clean();

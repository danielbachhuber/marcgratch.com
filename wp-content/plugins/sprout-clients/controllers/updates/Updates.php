...
class SC_Updates extends SC_Controller {
	...
	public static function register_settings() {
		...
		$settings = array(
			'sc_activation' => array(
				'title' => self::__( 'Sprout Clients Activation' ),
				'weight' => 0,
	...
	public static function license_key_option() {
		ob_start(); ?>
			...
			<?php if ( self::$license_status != false && self::$license_status == 'valid' ) : ?>
				<button id="sc_activate_license" class="button" disabled="disabled"><?php self::_e( 'Activate License' ) ?></button> 
				<button id="sc_deactivate_license" class="button"><?php self::_e( 'Deactivate License' ) ?></button>
			<?php else : ?>
				<button id="sc_activate_license" class="button button-primary"><?php self::_e( 'Activate License' ) ?></button>
			<?php endif ?>
	...
	public static function maybe_check_license() {
		...
		$message = ( $is_valid ) ? self::__( 'Thank you for supporting the future of Sprout Clients and Sprout Apps.' ) : self::__( 'License is not valid.' );
		$response = array(
				'valid' => $is_valid,
				'response' => $message,
			);

...
class SC_Clients_Admin_Table extends SC_Clients {
	...
	public static function restrict_contacts_by_status() {
		...
		if ( Sprout_Client::POST_TYPE === $typenow ) {
			...
			wp_dropdown_categories( array(
				'show_option_all' => sprintf( self::__( 'All %s' ), $status_taxonomy->label ),
				'taxonomy'        => $taxonomy,
				'name'            => $taxonomy,
				'orderby'         => 'name',
				'selected'        => $selected,
	...
	public static function setup_tribe_posts_manager() {
		...
		$filter_array = array(
			...
			'sc_facebook' => array(
				'name' => self::__( 'Facebook' ),
				'meta' => '_facebook',
			),
			'sc_linkedin' => array(
				'name' => self::__( 'Linkedin' ),
				'meta' => '_linkedin',
			),
			'sc_skype' => array(
				'name' => self::__( 'Skype' ),
			...
			'sc_twitter' => array(
				'name' => self::__( 'Twitter' ),
				'meta' => '_twitter',
			),
			'sc_website' => array(
				'name' => self::__( 'Website' ),
				'meta' => '_website',
			),
		);
	...
	public static function register_columns( $original_columns ) {
		...
		$columns['title'] = self::__( 'Contact' );
		$columns['sc_client_type'] = self::__( 'Type' );
		$columns['sc_client_status'] = self::__( 'Statuses' );
		$columns['lead_info'] = self::__( 'Info' );
		$columns['contacts'] = self::__( 'Associated Contacts' );
		if ( class_exists( 'Sprout_Invoices' ) ) {

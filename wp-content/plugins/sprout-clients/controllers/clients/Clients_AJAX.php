...
class SC_Clients_AJAX extends SC_Clients {
	...
	public static function maybe_create_user() {
		...
		if ( ! isset( $_REQUEST['sa_user_nonce'] ) ) {
			$response['error_message'] = self::__( 'Forget something?' );
		}
		...
		if ( ! wp_verify_nonce( $nonce, self::SUBMISSION_NONCE ) ) {
			...
			if ( ! $compatibility_check_passed ) {
				$response['error_message'] = self::__( 'Not going to fall for it!' );
			}
		}

		if ( ! current_user_can( 'publish_posts' ) ) {
			$response['error_message'] = self::__( 'User cannot create new posts!' );
		}

		// Attempt to create a user
		if ( ! isset( $_REQUEST['sa_user_email'] ) || '' === $_REQUEST['sa_user_email'] ) {
			$response['error_message'] = self::__( 'An e-mail is required' );
		}

		$client = Sprout_Client::get_instance( $_REQUEST['sa_user_client_id'] );

		if ( ! is_a( $client, 'Sprout_Client' ) ) {
			$response['error_message'] = self::__( 'Client not found' );
		}
	...
	public static function maybe_save_user_note() {
		...
		update_usermeta( $_REQUEST['user_id'], SC_Users::NOTE, $_REQUEST['note'] );

		$return = array();
		$return['response'] = self::__( 'Saved' );
		wp_send_json( $return );
	}

...
if ( class_exists( 'SC_Internal_Records' ) ) {
 ...
 * @subpackage Records
 */
class SC_Internal_Records extends SC_Controller {
	...
	public static function init() {
		add_action( 'sc_new_record', array( __CLASS__, 'new_record' ), 10, 6 );
	...
	public static function new_record( $data = array(), $type = 'mixed', $associate_id = -1, $title = '', $author_id = 0, $encoded = true ) {
		...
		$post = array(
			...
			'post_status' => 'pending',
			'post_type' => SC_Record::POST_TYPE,
			'post_parent' => $associate_id
		);
		$id = wp_insert_post( $post );

		if ( $id && ! is_wp_error( $id ) ) {
			$record = SC_Record::get_instance( $id );
	...
	public static function attempt_associated_record_deletion( $post_id = 0 ) {
		...
		if ( get_post_type( $post_id ) !== SC_Record::POST_TYPE ) {
			global $wpdb;
			$record_ids = $wpdb->get_col( $wpdb->prepare( "SELECT ID FROM $wpdb->posts WHERE post_parent = %d AND post_type = '%s'", $post_id, SC_Record::POST_TYPE ) );
	...
	public static function display_table() {
		...
		$wp_list_table = new SC_Records_Table();
	...
	<div class="wrap">
		<h2>
			<?php self::_e( 'Sprout Invoices Records' ) ?>
		</h2>

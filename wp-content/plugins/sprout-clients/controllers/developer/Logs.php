 ...
 * @subpackage Records
 */
class SC_Dev_Logs extends SC_Controller {
	const LOG_TYPE = 'dev_log';
	const ERROR_TYPE = 'sc_error';
	...
	public static function init() {
		...
		// action to log
		add_action( 'sc_log', array( __CLASS__, 'log' ), 10, 2 );
		add_action( 'sc_error', array( __CLASS__, 'error' ), 10, 3 );
	...
	/**
	 * Utility function for sc_new_record
	 * @param  string  $subject      short title
	...
	/**
	 * Periodically query for logs older than sc_logs_purge_filter_delay (defaults to 15 days) and delete them.
	 *
	 * @return null
	 */
	...
	public static function filter_where_with_when( $where = '' ) {
		// posts 15+ old
		$offset = apply_filters( 'sc_logs_purge_filter_delay', date_i18n( 'Y-m-d', strtotime( '-15 days' ) ), $where );

		...
		<ul>
			<?php foreach ( $associated_users as $user_id ) :
				...
				<li class="associated_user clearfix">
					...
					<button data-id="<?php echo (int) $user_id ?>" data-client-id="<?php echo (int) $id ?>" class="unassociate_user button button-small casper"><?php self::_e( 'Remove' ) ?></button>

					<?php
						$full_name = ( '' === $a_user->first_name && '' === $a_user->last_name ) ? self::__( 'Mystery Man' ) : $a_user->first_name . '&nbsp;' . $a_user->last_name ;
							?>
					...
					<div class="sc_person_info clearfix">
						...
						<textarea id="user_note_field_<?php echo esc_attr( $user_id ) ?>" class="si_redactorize sc_user_note" data-user-id="<?php echo (int) $user_id ?>" placeholder="<?php self::_e( 'Save a note about this person.' ) ?>"><?php echo esc_textarea( sc_get_users_note( $user_id ) ) ?></textarea>
						
						<button id="submit_user_note_<?php echo esc_attr( $user_id ) ?>" class="submit_user_note button button-mute" data-user-id="<?php echo esc_attr( $user_id ) ?>"><?php echo self::_e( 'Save Note' ) ?></button>
					</div>
				</li>
			<?php endforeach ?>
		</ul>
...
<div id="associated_users_mngt">
	<select id="associated_users" class="select2" data-nonce="<?php echo wp_create_nonce( SC_Controller::NONCE ) ?>">
		<option></option>
		...
		<div id="associate_user_select_help" class="sa-dropdown sa-dropdown-tip sa-dropdown-relative">
			<div class="sa-dropdown-panel">
				<?php self::_e( 'Contacts can have multiple people associated; consider a company/client that may have multiple contacts. Each person may receive notifications depending on what is sent.' ) ?>
			</div>
		</div>
	...
	<a href="#TB_inline?width=300&height=250&inlineId=user_creation_modal" id="user_creation_modal_tb_link" class="thickbox button" title="<?php sc_e( 'Create new user for this lead' ) ?>"><?php sc_e( 'New Person' ) ?></a>&nbsp;<span class="helptip add_user_select_help" data-sa-dropdown="#add_user_select_help"></span>
		<div id="add_user_select_help" class="sa-dropdown sa-dropdown-tip sa-dropdown-relative">
			<div class="sa-dropdown-panel">
				<?php self::_e( '"People" are WordPress users set with the role of client, they have no permissions by default but providing your contacts a user (which they won\'t know about by default) allows for a whole slew of features and flexibility for customizations.' ) ?>
			</div>
		</div>
</div>